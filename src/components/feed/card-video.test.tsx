 ...
 * NO navegue —son los que impiden que `/videos` vuelva a colarse por NAVEGACIÓN
 * en el gesto de la tarjeta—, y que el visor siga siendo el respaldo cuando el
 * reel no tiene ese video (y `/videos`, el respaldo del respaldo).
 *
 * Que el visor sea el RESPALDO no lo hace menos exigente: todo lo que se le
 * pedía cuando era el destino del toque se le sigue pidiendo acá abajo sobre
 * ese camino — el video completo, el segundo heredado de la tarjeta y las
...
describe("CardVideo: un toque abre el REEL, encima del feed", () => {
  ...
  it("el respaldo hereda el segundo donde venía la tarjeta, no vuelve a cero", () => {
    // Quien tocó a los 12 s no puede aterrizar en el principio del clip: el
    // visor abre en `startSeconds`, que sale del `currentTime` de la tarjeta.
    renderCard();
